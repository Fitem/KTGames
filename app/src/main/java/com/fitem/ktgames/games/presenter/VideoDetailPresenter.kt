...
import com.fitem.ktgames.games.model.VideoDetailModel
...
class VideoDetailPresenter : BasePresenter<VideoDetailContract.View>(), VideoDetailContract.Presenter {
    ...
    override fun loadVideoInfo(itemInfo: HomeBean.Issue.Item) {
        ...
        val netType = NetworkUtil.isWifi(AppApplication.context)
        // 检测是否绑定 View
        checkViewAttached()
        if (playInfo!!.size > 1) {
            ...
            if (netType) {
                ...
                for (i in playInfo) {
                    if (i.type == "normal") {
                        val playUrl = i.url
                        mRootView?.setVideo(playUrl)//Todo 待完善
                        ToastUtils.showShort("本次消耗${FormatUtils.dataFormat(i.urlList[0].size)}流量")
                        break
                    }
                }
            }
        ...
        //设置背景
        val backgroundUrl =
            itemInfo.data.cover.blurred + "/thumbnail/${ScreenUtils.getScreenHeight() - ConvertUtils.dp2px(250f)!!}x${ScreenUtils.getScreenWidth()}"
