...
class NewsListAdapter(layoutResId: Int, data: List<GNews>) :
    ...
    override fun convert(helper: BaseViewHolder?, item: GNews?) {
        ...
        val color = if (isFirst) accentColor else titleColor

        helper?.let {
            it.setTextColor(R.id.tv_title, color)
            it.setTextColor(R.id.tv_date, color)
            it.setTextColor(R.id.tv_source, color)
